...
为什么需要它：`src/parts/court.js` 里一处非法的嵌套引号让整个 IIFE 抛异常，
`profile` / `profileLine` / `TONES` 全部没定义，今日页渲染成空白——
而 a11y / budget / tokens 三道门禁**全部报 PASS**。它们检查的是产物的形状
（颜色、体积、字面值），没有一道检查产物**跑不跑得起来**。

这是本仓库第三次撞上同一个模式：机器门禁只查形状不查真值。
内容层是出处，设计层是颜色，这里是可执行性。
...
J2 是第四次撞上「只查形状不查真值」之后补的：emoji→图标 的整批替换把
`return 📖 …` 改成了 `returnic('book')+' …'`，少了一个空格。
`returnic(...)` 语法完全合法——它只是调用一个不存在的函数——所以 node --check
一路 PASS，一到运行就 ReferenceError，整条 canonLabel 挂掉。
