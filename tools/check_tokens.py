...
为什么需要它：design/tokens.json 一直是「单一真源」，build.py 也一直在生成
CSS 变量，但 src/template.html 自建了一套简写变量并用字面 hex 赋值，于是
页面上的颜色和令牌里的颜色可以长期不一致。最典型的一例是 text.meta：
tokens 记录「已修复为 mist-400（7.80:1）」，页面实际画的是 #6f688a（3.24:1），
而 check_a11y 读的是 tokens，所以一路 PASS。令牌不被强制引用，真源就不是真源。
